...
export default function Aprobaciones({ user }) {
  ...
  const cargar = async () => {
    ...
    snap.forEach(d => {
      ...
      if (data.estado === "pendiente") pend.push(data);
      else if (data.estado === "aprobado") apr.push(data);
    });
    setPendientes(pend);
    setAprobados(apr);
    setLoading(false);
          ...
          {pendientes.map(u => (
            <div key={u.id} className="glass-panel p-4 flex items-center justify-between animate-slide-up">
              ...
              <div className="flex gap-2">
                <button 
                  onClick={() => cambiarEstado(u.id, u.email, "aprobado")}
                  className="flex items-center gap-1 px-3 py-2 rounded-lg bg-green-500/10 text-green-500 hover:bg-green-500 hover:text-white transition-all font-medium text-sm"
                >
                  <UserCheck size={16} /> Aprobar
                </button>
                <button 
                  onClick={() => cambiarEstado(u.id, u.email, "rechazado")}
                  className="flex items-center gap-1 px-3 py-2 rounded-lg bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white transition-all font-medium text-sm"
                >
